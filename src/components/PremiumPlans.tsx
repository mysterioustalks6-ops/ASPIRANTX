      ...
      if (orderData.hasKey && orderData.keyId && typeof window !== 'undefined') {
        ...
        if (isRazorpayValidFn()) {
          const options: any = {
            ...
            name: 'StudyRide Pro Membership',
            description: `StudyRide PRO ${selectedPlan.toUpperCase()} Pass`,
            prefill: {
              name: user?.name || 'Aspirant Student',
              email: user?.email || 'student@studyride.in',
            },
            theme: { color: '#f59e0b' },
              ...
              {paymentSuccess ? (
                <div className="py-8 text-center space-y-4">
                  ...
                  <p className="text-xs text-slate-300">
                    Your StudyRide PRO Pass is now active. Enjoy full access to AI Chatbots, PYQ Predictors, and Answer Evaluation!
                  </p>
                  <button
                    onClick={() => {
                      setShowPaymentModal(false);
        ...
        <div className="p-6 rounded-3xl bg-slate-900/90 border-2 border-amber-500/50 relative overflow-hidden flex flex-col justify-between shadow-2xl shadow-amber-950/20">
          ...
          <div>
            <span className="text-xs font-bold uppercase tracking-wider text-amber-400 flex items-center gap-1.5">
              <Sparkles className="w-4 h-4" /> StudyRide PRO {selectedPlan.toUpperCase()} Pass
            </span>
