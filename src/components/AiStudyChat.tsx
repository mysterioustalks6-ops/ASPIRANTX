...
export const AiStudyChat: React.FC<AiStudyChatProps> = ({ exam, userId, userEmail }) => {
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        ...
        let fallbackText = `**StudyRide AI Study Mentor (${exam} Guidance):**\n\n`;
  ...
  const handleExportMarkdown = () => {
    const mdContent = `# StudyRide AI Mentor Session: ${
    ...
    a.href = url;
    a.download = `StudyRide_AI_Session_${Date.now()}.md`;
    a.click();
