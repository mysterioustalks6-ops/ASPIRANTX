...
export const LibraryEngine: React.FC<LibraryEngineProps> = ({ user, onNavigate }) => {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Curated standard textbooks, old/new NCERT collections, and official syllabus reference manuals.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            <Bookmark className={`w-3.5 h-3.5 ${showBookmarksOnly ? 'fill-sky-400 text-sky-400' : ''}`} />
          ...
          {isAdmin && (
            <button
              ...
              className="px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs rounded-xl shadow-md shadow-sky-600/25 transition-all flex items-center gap-1.5 cursor-pointer"
            >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 sm:p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="md:col-span-2 relative">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-sky-500 transition-colors font-medium"
            />
          </div>

          {/* Read-Only Active Exam Indicator */}
          <div className="relative">
            <div className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-sky-300 font-bold flex items-center justify-between">
              <span>{selectedExam === 'ALL' ? 'Active Profile Exam' : selectedExam.replace(/_/g, ' ')}</span>
              <span className="text-[8px] bg-sky-500/10 text-sky-400 border border-sky-500/20 px-1.5 py-0.5 rounded uppercase font-semibold">Active Context</span>
            </div>
          </div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2.5 text-xs text-slate-300 focus:outline-none focus:border-sky-500 font-medium cursor-pointer"
            >
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1.5 scrollbar-thin">
          <span className="text-[10px] text-slate-500 font-bold uppercase shrink-0">Subjects:</span>
          {subjectsList.map(sub => (
            <button
              key={sub}
              onClick={() => setSelectedSubject(sub)}
              className={`px-3 py-1 rounded-full text-[10px] font-bold transition-all border shrink-0 cursor-pointer ${
                selectedSubject === sub
                  ? 'bg-sky-600 text-white border-sky-500 shadow-sm shadow-sky-600/25'
                  : 'bg-slate-950 text-slate-400 border-slate-800 hover:text-white'
              }`}
            >
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between pl-2">
                    <span className="text-[9px] bg-slate-950 border border-slate-800 text-slate-400 px-2 py-0.5 rounded font-bold uppercase tracking-wider">
                      {book.category}
                    </span>
                  ...
                  <div className="flex items-start gap-4 pl-2">
                    ...
                    <div className={`w-14 h-20 rounded-lg shadow-md flex items-center justify-center shrink-0 border border-slate-800 p-1 relative ${book.coverColor || 'bg-slate-950'}`}>
                      <div className="absolute left-1 top-0 bottom-0 w-[2px] bg-black/40" />
                      <span className="text-[8px] font-bold text-white text-center leading-tight line-clamp-3">
                        {book.title}
                      </span>
                    </div>

                    <div className="space-y-1">
                      <h3 className="font-extrabold text-white text-sm leading-snug group-hover:text-sky-400 transition-colors">
                        {book.title}
                      </h3>
                      <p className="text-[11px] text-slate-400 font-medium">By {book.author}</p>
                      {book.edition && (
                  ...
                  {/* Book details */}
                  <p className="text-xs text-slate-300 leading-relaxed font-normal line-clamp-3 pl-2">
                    {book.description}
                  </p>
                  ...
                  {book.mappedTopics && book.mappedTopics.length > 0 && (
                    <div className="space-y-1 pl-2">
                      <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider">Mapped Topics:</span>
                      <div className="flex flex-wrap gap-1.5">
                        {book.mappedTopics.map(topic => (
                          <span 
                            key={topic}
                            onClick={() => setSearchQuery(topic)}
                            className="px-2 py-0.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-[9px] text-sky-400 hover:text-sky-300 rounded font-semibold cursor-pointer transition-colors"
                          >
                            {topic}
                          </span>
                        ))}
                ...
                {/* Actions panel */}
                <div className="pt-4 mt-4 border-t border-slate-800/80 flex items-center justify-between pl-2">
                    ...
                    {isAdmin && (
                      <>
                        ...
                        <button
                          ...
                          className="p-1.5 hover:bg-rose-500/10 text-slate-400 hover:text-rose-400 rounded-lg transition-colors border border-slate-800 cursor-pointer"
                        >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {downloadingId === book.id ? (
                      <div className="w-24 space-y-1.5">
                        ...
                        <div className="w-full bg-slate-950 h-1 rounded-full overflow-hidden">
                          <div className="bg-sky-500 h-full transition-all" style={{ width: `${downloadProgress}%` }} />
                        </div>
                      </div>
                    ) : (
                      <button
                        onClick={() => handleDownloadNotes(book.id)}
                        className="px-3 py-1.5 bg-slate-950 border border-slate-800 hover:border-sky-500/30 text-slate-300 hover:text-white font-bold text-[10px] rounded-xl transition-all flex items-center gap-1 cursor-pointer"
                      >
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-sky-600 hover:bg-sky-500 text-white font-bold text-[10px] rounded-xl shadow-md shadow-sky-600/25 transition-all flex items-center gap-1 cursor-pointer"
                    >
