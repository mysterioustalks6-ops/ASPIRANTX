...
/**
 * Fetches official syllabus hierarchy nodes for an exam
 */
...
/**
 * Fetches personal syllabus nodes for a user and exam
 */
export async function fetchPersonalSyllabus(userId?: string, exam?: string): Promise<PersonalSyllabusNode[]> {
  try {
    const url = `/api/personal-syllabus?userId=${encodeURIComponent(userId || 'guest')}${exam ? `&exam=${encodeURIComponent(exam)}` : ''}`;
    ...
    console.warn('API fetch personal syllabus failed, fallback to local helper:', e);
  }
  return getPersonalSyllabusNodes(userId, exam);
}
